...
### HTTPS и WebSocket (КРИТИЧНО!)

GitHub Pages работает по **HTTPS**, а браузеры блокируют смешанный контент. Это означает:
- ❌ Нельзя использовать `ws://` (небезопасный WebSocket) с HTTPS страницей
- ✅ Нужно использовать `wss://` (безопасный WebSocket) или настроить туннель

### Решения:

#### Вариант 1: Использовать туннель (рекомендуется для локального сервера)

**С ngrok:**
```bash
# Установите ngrok: https://ngrok.com/download
ngrok http 3000
# Используйте HTTPS URL из вывода (например: https://abc123.ngrok.io)
# В GitHub Secrets установите: VITE_WS_URL=wss://abc123.ngrok.io/ws
```

**С Cloudflare Tunnel (бесплатно):**
```bash
...
# Используйте HTTPS URL из вывода
```

#### Вариант 2: Настроить SSL на локальном сервере

Для локального IP это сложно, так как нужен SSL сертификат. Можно использовать самоподписанный сертификат, но браузеры будут показывать предупреждение.

#### Вариант 3: Использовать домен с SSL

Если у вас есть домен, настройте SSL сертификат (Let's Encrypt) и используйте `wss://your-domain.com/ws`
